...
/*   Updated: 2026/02/11 11:46:08 by vvazzs           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_minimap_tile_one(t_gen *gen, int x, int y, int color)
{
	...
	py = 0;
	while (py < MINIMAP_TILE_PIXELS)
	{
		px = 0;
		while (px < MINIMAP_TILE_PIXELS)
		{
			copied_mlx_pixel_put(gen->img_data, x * MINIMAP_TILE_PIXELS + px, y
				* MINIMAP_TILE_PIXELS + py, color);
			px++;
		}
		py++;
	}
